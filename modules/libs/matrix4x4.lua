...
function mat4:constructor(m)
	self.data = love.data.newByteData(mat4.bytesize)
...
function mat4:set_position_and_rotation(position, angle, axis)
	self._changed = true
...
function mat4:shear(yx, xy, zx, zy, xz, yz)
	self._changed = true
	identity(temp_array)
...
function mat4:multiply_vec3(v, out)
	out = out or v
	local e = self.e
...
function mat4:is_changed()
	return self._changed == true
end

function mat4.is_mat4(t)
	return type(t) == "table"
end
...
function mat4.__newindex(t, k, v)
	if type(k) == "number" then
		t.e[k] = v
	else
		rawset(t, k, v)
